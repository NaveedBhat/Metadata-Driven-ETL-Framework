...
---------------------
Worker DAG that handles the end-to-end ingestion, validation, transformation,
and loading of any single file configuration defined in SOURCE_CONFIG.

It expects to be triggered by a Master DAG, receiving a `config_id` via `dag_run.conf`.
"""
...
from alerts import task_failure_alert, dag_success_alert
...
def fetch_metadata(**context):
    ...
    if not config_id:
        raise ValueError("Missing config_id in dag_run.conf. This DAG must be triggered with a config_id.")
    ...
    try:
        ...
        metadata = {
            ...
            # Used in transform_file to separate schema validation from null checks.
            "nullable_columns": row[9] or "",
        }
        
        context["ti"].xcom_push(key="source_metadata", value=metadata)
        logger.info(
            "Metadata loaded for table: %s | nullable cols: [%s]",
...
def download_file(**context):
    ...
    - XLSX      : Google Sheets files are exported as CSV via the Drive
                  export API; uploaded XLSX binaries are read with
                  pd.read_excel() and then written out as CSV.
                  Either way the raw_file_path on disk is always a CSV so
    ...
    if file_format == "XLSX":
        ...
        # binary XLSX.  Both paths produce a CSV on disk so the downstream
        # tasks don't need to know the original format.
        # ----------------------------------------------------------------
        try:
...
def transform_file(**context):
    ...
    - NULLABLE_COLUMNS: Subset of COLUMN_LIST that are allowed to be NULL. Only
                       the remaining columns (mandatory ones) trigger row rejection
                       when null values are found.

    This means a record is only rejected when a column that is genuinely required
    for business logic is missing — not because an optional field like manager_id
    or review_text happens to be blank.
...
def load_to_snowflake(**context):
    """Loads the clean file to Snowflake and logs to ETL_LOG."""
    ...
    try:
        ...
        if success:
            ...
            try:
                ...
                conn.commit()
                logger.info("Set LOADED_AT for %d rows in %s", insertion_rowcount, table_name.upper())
            finally:
                update_cursor.close()
        else:
        ...
        try:
            ...
            conn.commit()
            logger.info("Logged run to %s (status=%s, config_id=%s)", SNOWFLAKE_LOG_TABLE, table_load_status, config_id)
